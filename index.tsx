 ...
 * Ver.3.1 (UX改善版)
 * 
 * このバージョンでは、プレイヤーの体験を向上させるため、
 * 持ち時間を100秒に延長し、時間切れメッセージをより丁寧なものに変更しています。
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // =================================================================
  // Game Configuration (★変更点)
  // =================================================================
  const PLAYER_TOTAL_TIME_SECONDS = 100; // 60秒から100秒に延長
  ...
  function findBestAiResponse(rawResponse: string, requiredKana: string, usedWords: string[]): string | null {
    ...
    for (const word of shuffledWords) {
      ...
      if (normalizedFirstChar === normalizedRequiredKana) return word;
    }
    return null;
  }
  ...
  // =================================================================
  // UI, Timers, and Game State Functions
  // =================================================================
  function initializeGame() { startButton.addEventListener('click', startGame); playerInput.addEventListener('keypress', (event) => { if (event.key === 'Enter' && !playerInput.disabled) { handlePlayerSubmit(); } }); loadHighScore(); };
  ...
  function startGame() { gameOver = false; wordHistory = []; currentWord = ''; currentLinkingKana = ''; isPlayerTurn = false; playerTypedCharactersCount = 0; clearTimers(); playerTimeRemaining = PLAYER_TOTAL_TIME_SECONDS; playerTimeRemainingDisplay.textContent = playerTimeRemaining.toString(); gameArea.classList.remove('hidden'); scoreDisplayArea.classList.add('hidden'); startButton.textContent = 'リセットして再挑戦'; startButton.disabled = false; updateWordHistoryUI(); displayMessage('ゲーム開始！最初の言葉(2文字以上)を入力してください。', false); currentWordTextDisplay.textContent = '---'; previousWordDisplay.textContent = 'なし'; nextKanaDisplay.textContent = '?'; setTurn(true); };
